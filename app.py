...
# Upload first - ONLY this shows initially
...
if uploaded_file is not None:
    ...
    # Default values (only initialize once)
    ...
    # Layout: Original image LEFT, Controls + Simulation RIGHT
    ...
    with right_col:
        st.subheader("⚙️ Ajustes y vista previa")
        
        # Sliders stacked vertically on the right
        ancho_puntadas = st.slider(
            "Ancho del patrón (puntos)", 20, 200, st.session_state.ancho_puntadas, 5,
            key="slider_ancho", help="Más puntos = más detalle, patrón más grande"
        )
        st.session_state.ancho_puntadas = ancho_puntadas
        
        numero_colores = st.slider(
            "Número de colores", 2, 30, st.session_state.numero_colores, 1,
            key="slider_colores", help="Menos colores = más fácil de bordar"
        )
        st.session_state.numero_colores = numero_colores
        
        tamano_bloque = st.slider(
            "Tamaño de punto en PDF (px)", 20, 50, st.session_state.tamano_bloque, 2,
            key="slider_tamano", help="Más grande = más fácil de ver"
        )
        st.session_state.tamano_bloque = tamano_bloque
        
        margen = st.slider(
            "Margen de página (px)", 30, 100, st.session_state.margen, 5,
            key="slider_margen", help="Espacio para numeración y anotaciones"
        )
        st.session_state.margen = margen
        
        mostrar_bn = st.checkbox(
            "Incluir versión blanco y negro", value=st.session_state.mostrar_bn,
            key="check_bn", help="Añade 4 páginas extra con solo símbolos"
        )
        st.session_state.mostrar_bn = mostrar_bn
        
        st.markdown("---")
        
        # Simulation preview (updates live with sliders)
        st.caption("🧵 Patrón cuantizado (simulación en tiempo real)")
    ...
    # No image uploaded - show only upload prompt and PDF info
    st.info("👆 Sube una imagen para ver los ajustes y la vista previa")
